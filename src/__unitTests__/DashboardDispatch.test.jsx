...
test('Dispatch dashboard does not render invalid tile', () => {
    render(
        <MemoryRouter>
            <DashboardDispatch allowedTiles={['invalidTile', 'manageJobs', 'addJobListing', 'viewChanges','viewJobBoard']} />
        </MemoryRouter>
    )
    // valid tiles renders
    expect(screen.getByText(/Manage Jobs/i)).toBeInTheDocument()
...
test('Dispatch Dashboard tiles have valid links', () => {
  render(
    <MemoryRouter>
      <DashboardDispatch allowedTiles={['viewChanges', 'viewJobBoard', 'addJobListing', 'manageJobs' ]} />
    </MemoryRouter>
  )
