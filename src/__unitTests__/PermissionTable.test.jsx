...
describe('Use Permission Functions', () => {
    describe('matchPattern Functionality', () => {
        ...
        function matchPattern(pattern, path) {
            ...
            if (pattern.includes('*')) {
                ...
                const regexStr =
                    '^' + escapeForRegex(pattern).split('\\*').join('.*') + '$'
    ...
    describe('isRouteAllowed Functionality', () => {
        test('Single Role Test', () => {
            expect(isRouteAllowed('dispatch', '/manage-jobs')).toBe(true)
        })

        test('Array of Roles Test', () => {
            expect(
                isRouteAllowed(['admin', 'dispatch', 'display'], '/login')
            ).toBe(true)
        })
        ...
        test('Unknown Role Fallback Test', () => {
            expect(isRouteAllowed('unknown', '/manage-jobs')).toBe(false)
        })
