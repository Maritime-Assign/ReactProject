        ...
        <div className='w-full pt-4 flex flex-col max-w-[1280px] mx-auto font-mont'>
            <div className='flex py-4 bg-mebablue-dark rounded-md w-full shadow-xl relative items-center'>
                ...
                <Link className='flex gap-2 mr-4' to={'/add-user'}>
                    <button className="bg-mebablue-light hover:bg-mebablue-hover p-2 rounded text-white" title="Add User">
            ...
            <div className=" bg-white rounded-lg shadow overflow-hidden mt-6">
                <div className="grid grid-cols-[80px_180px_250px_320px_1fr] items-center px-4 py-2 bg-blue-50">
                    ...
                    <div className='px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider'>Role
                        <span className="info-icon" title="Roles: Admin, Dispatch, Display">
                            🔍
                        </span>
                    </div>
                    <div className='px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider'>Abbreviation</div>
                    <div className='px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider'>Name</div>
                    <div className='px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider'>Username</div>
                </div>
                {sortedUsers.map((user) => (
                    <div className="grid grid-cols-[80px_180px_250px_320px_1fr] items-center px-4 py-2 bg-white border-b border-gray-200" key={user.id}>
                        <div className="px-6 py-4 text-sm text-gray-900 truncate">
                        {/*Button that links to the edit user page*/}
                            <Link to={'/edituser'} state={user}>
                                <button className="text-mebablue-dark hover:text-mebablue-hover edit role" title="Edit Role">
                                    <FaEdit />
                                </button>
                            </Link>
                        </div>
                        <div className="px-6 py-4 text-sm text-gray-900 truncate">{user.role[0].toUpperCase() + user.role.slice(1)}</div>
                        <div className="px-6 py-4 text-sm text-gray-900 truncate">{user.abbreviation}</div>
                        <div className="px-6 py-4 text-sm text-gray-900 truncate">{user.first_name}</div>
                        <div className="px-6 py-4 text-sm text-gray-900 truncate">{user.username}</div>
                    </div>
                ))}
            </div>
        </div>
