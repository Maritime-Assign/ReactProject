...
import { logJobClaim } from './AuditFetch'
...
const JobListing = ({ rowIndex, handleClaimJob, ...props }) => {
    ...
    // Claim a job function with audit logging
    const claimJob = async () => {
        ...
        try {
            ...
            if (updateError) {
                ...
                // Successfully claimed - log the action
                try {
                    await logJobClaim(props.id);
                } catch (auditError) {
                    console.error('Failed to log job claim:', auditError);
                    // Don't show error to user as the claim was successful
                }
                
                handleClaimJob();
            } else {
                setError('Job may have already been claimed by another user.');
            }
        } catch (err) {
            console.error('Exception during job claim:', err);
